...
class ProjectTabWidget(QTabWidget):
    ...
    def debug_timers(self):
        ...
        print("All floating timers:")
        for i, timer in enumerate(self.all_floating_timers):
            print(f"  {i}: {timer.project_label.text() if hasattr(timer, 'project_label') else 'Unknown'} - Visible: {timer.isVisible()}")

        print("Project timer references:")
        for i, project in enumerate(self.project_tabs):
            timer = project.get('floating_timer')
            print(f"  Project {i} ({project.get('name', 'Unknown')}): Timer {timer.project_label.text() if timer and hasattr(timer, 'project_label') else 'None'} - Visible: {timer.isVisible() if timer else 'N/A'}")

    def check_for_inconsistencies(self):
        """Check for inconsistencies between project tabs and timers"""
        print("===== CHECKING FOR INCONSISTENCIES =====")
        print(f"Project count: {len(self.project_tabs)}")
        print(f"Timer count: {len(self.all_floating_timers)}")

        if len(self.all_floating_timers) != len(self.project_tabs):
            print("WARNING: Mismatch between project count and timer count!")

        # Check for projects without timers
        projects_without_timers = [i for i, project in enumerate(self.project_tabs) if not project.get('floating_timer')]
        if projects_without_timers:
            print("Projects without timers:")
            for i in projects_without_timers:
                print(f"  Project {i}: {self.project_tabs[i].get('name', 'Unknown')}")

        # Check for orphaned timers
        orphaned_timers = [timer for timer in self.all_floating_timers if timer not in [p.get('floating_timer') for p in self.project_tabs]]
        if orphaned_timers:
            print("Orphaned timer IDs:")
            for timer in orphaned_timers:
                print(f"  {timer.project_id if hasattr(timer, 'project_id') else 'Unknown'}")
